...
export function enrichError(
  ...
  // Handle case where error is a plain object with message property
  let originalError: Error;
  if (error instanceof Error) {
    originalError = error;
  } else if (typeof error === 'object' && error !== null && 'message' in error) {
    // It's an error-like object (e.g. from some libs or serialized errors)
    const errObj = error as { message: string; stack?: string; name?: string; code?: string };
    originalError = new Error(errObj.message);
    if (errObj.stack) originalError.stack = errObj.stack;
    if (errObj.name) originalError.name = errObj.name;
    // Attach original object's properties to the new Error if needed, or just rely on classification
  } else {
