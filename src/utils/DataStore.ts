...
export class DataStore<T> {
    ...
    try {
      await this.performWithRetry(async () => {
        ...
        if (await this.fileExists(this.filePath)) {
          await this.performBackup('before_save');
        }
  ...
  async backup(reason = 'manual'): Promise<string> {
    const release = await this.mutex.acquire();
    try {
      return await this.performWithRetry(() => this.performBackup(reason));
    } finally {
      release();
    }
  }
  ...
  /**
   * Internal backup implementation without mutex locking
   * Used by methods that already hold the lock (save, restore, migrate)
   */
  private async performBackup(reason: string): Promise<string> {
    if (!(await this.fileExists(this.filePath))) {
  ...
  async restore(backupPath: string): Promise<void> {
    ...
    try {
      await this.performWithRetry(async () => {
        ...
        if (await this.fileExists(this.filePath)) {
          await this.performBackup('before_restore');
        }
  ...
  async migrate(migrationFn: MigrationFunction<T>): Promise<void> {
    ...
    try {
      await this.performWithRetry(async () => {
        ...
        if (currentData !== null) {
          await this.performBackup('before_migration');
        }
