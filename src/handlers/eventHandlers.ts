...
async function handleInteractionCreate(
  ...
  // Import error handling utilities - will be available from ErrorHandlingUtils
  const { enrichError, isRetryableError, getUserFriendlyMessage, createTimeoutPromise } = await import('../utils/ErrorHandlingUtils');
  
  try {
    // Prevent concurrent command execution by same user with timeout
      ...
      const commandPromise = (async () => {
        ...
        default:
          throw enrichError(new Error('Command not found'), {
            ...
            commandName,
            availableCommands: ['chat', 'status', 'clear', 'remember', 'addgag', 'setpersonality', 'mypersonality', 'getpersonality', 'removepersonality', 'clearpersonality', 'execute', 'contextstats', 'summarize', 'deduplicate', 'crossserver', 'ascii']
          });
        }
      })();
    ...
    try {
      if (interaction.deferred) {
        ...
        // Fallback to followUp if interaction was already replied to
    ...
    // Note: This would typically call analyticsManager.trackCommandError but we'll handle safely
    try {
      ...
      // Remove bot mention from message content to get actual user prompt
        ...
        try {
          ...
          const response = await Promise.race([
            ...
            createTimeoutPromise(60000).then(() => {
