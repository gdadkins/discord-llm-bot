...
/**
 * Interface for local user analysis component
 */
export interface ILocalUserAnalyzer {
  /**
...
export interface LocalAnalysisResult {
  ...
  capsUsage: number; // percentage
  avgWordsPerMessage: number;
  linkCount: number;
  mentionCount: number;
  
  // Conversation style indicators
  shortMessageRatio: number; // % of messages under 10 words
  longMessageRatio: number; // % of messages over 50 words
  
  // Interest signals
  techTermFrequency: Map<string, number>;
  topicSignals: Map<string, number>;
  
  // Should we call API for deeper analysis?
  requiresApiAnalysis: boolean;
  apiAnalysisReasons: string[];
}
