...
import type { User, Message, GuildMember, Guild } from 'discord.js';
import type { IAIService } from '../../interfaces/AIServiceInterfaces';
import type { UserAnalysisResult, ApiAnalysisResult } from '../../interfaces/UserAnalysisInterfaces';
import type { LocalAnalysisResult } from '../../interfaces/AnalyticsInterfaces';
...
const topicRoasts: Record<string, string> = {
  'gaming': 'needs to touch grass immediately',
  'code': 'probably dreams in binary',
  'tech': 'spends more on gear than soap',
  'anime': 'has a waifu pillow for sure',
  'crypto': 'is probably broke',
  'nft': 'makes bad financial decisions',
  'politics': 'must be fun at parties',
  'food': 'personality is just consumption',
  'gym': 'we get it, you lift',
  'music': 'thinks their taste is unique (it isn\'t)'
};

const styleRoasts: Record<string, string> = {
  'aggressive': 'needs anger management',
  'passive': 'is terrified of confrontation',
  'formal': 'talks like a corporate email',
  'chaotic': 'types with their face',
  'emoji': 'uses pictures because words are hard',
  'short': 'can\'t hold a conversation',
  'long': 'writes essays nobody reads'
};

const interestRoasts: Record<string, string> = {
  'league': 'toxic and proud',
  'valorant': 'clicks heads because real life is hard',
  'genshin': 'gambling addict in denial',
  'minecraft': 'never grew up',
  'fortnite': 'is mentally 12',
  'roblox': 'stole mom\'s credit card'
};
