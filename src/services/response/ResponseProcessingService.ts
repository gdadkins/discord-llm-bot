...
// Define missing types locally if not exported
interface ProcessingResult {
  // Define structure based on usage or use any
  [key: string]: any;
}
...
export class ResponseProcessingService implements IResponseProcessingService {
  ...
  async processAPIResponse(
    ...
    try {
      ...
      if (config.includeThoughts && thinkingText && config.thinkingBudget > 0) {
        ...
        const thinkingMetadata: PerformanceMetadata = {
      ...
      if (candidate.content?.parts && candidate.content.parts.length > 0) {
        ...
        parts.forEach((part: any, index: number) => {
          ...
          const isThinkingPart = part.thought === true || 
                                part.role === 'model-thinking' ||
                                part.role === 'thinking';

          if (isThinkingPart && part.text) {
            logger.debug(`Part ${index} identified as thinking content (thought=${part.thought}, role=${part.role})`);
  ...
  processMultimodalResponse(
    ...
    const videoAttachments = config.processedAttachments.filter((a: any) => a.contentType === 'video');
    const imageAttachments = config.processedAttachments.filter((a: any) => a.contentType !== 'video');
    ...
    videoAttachments.forEach((attachment: any) => {
    ...
    config.processedAttachments.forEach((attachment: any, index: number) => {
