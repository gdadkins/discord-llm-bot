...
import Ajv from 'ajv';
import { BotConfiguration, IConfigurationValidator } from '../interfaces/ConfigurationInterfaces';
...
export class ConfigurationValidator implements IConfigurationValidator {
  ...
  /**
   * Validate a complete BotConfiguration object
   */
  ...
  /**
   * Validate schema evolution - check if new schema is compatible with old data
   */
  validateSchemaEvolution(oldSchema: object, newSchema: object): ValidationResult {
    const errors: ValidationError[] = [];
    const warnings: string[] = [];

    // Basic schema compatibility checks
    try {
      // Check if old schema properties still exist in new schema
      const oldProps = (oldSchema as { properties?: Record<string, unknown> }).properties || {};
      const newProps = (newSchema as { properties?: Record<string, unknown> }).properties || {};
      ...
      for (const prop in oldProps) {
        if (!(prop in newProps)) {
          errors.push({
            field: prop,
            value: undefined,
            message: `Property '${prop}' removed in new schema`,
            expected: 'Property should be maintained or deprecated'
          });
        }
      }
      ...
      for (const field of newRequired) {
        if (!oldRequired.includes(field)) {
          warnings.push(`New required field '${field}' added - may break existing configurations`);
        }
      }
   ...
   */
  validateMigration(fromConfig: BotConfiguration, toConfig: BotConfiguration): ValidationResult {
    const errors: ValidationError[] = [];
    const warnings: string[] = [];
    ...
    if (fromConfig.version === toConfig.version) {
      errors.push({
        field: 'version',
        value: toConfig.version,
        message: 'Configuration version should be updated during migration',
        expected: 'New version identifier'
      });
    }
   ...
   */
  private getNestedValue(obj: BotConfiguration, path: string): unknown {
