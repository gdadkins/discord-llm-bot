...
import { getGeminiConfig, GEMINI_MODELS } from './GeminiConfig';
...
export class GeminiAPIClient implements IGeminiAPIClient {
  ...
  constructor(
    ...
    const config = geminiConfig as any;
    
    this.config = {
      systemInstruction: config.systemInstruction || '',
      groundingThreshold: config.groundingThreshold || 0.5,
      ...
      forceThinkingPrompt: config.forceThinkingPrompt || false,
      thinkingTrigger: config.thinkingTrigger || '',
      enableGoogleSearch: config.enableGoogleSearch || false,
      unfilteredMode: config.unfilteredMode || false
    };
