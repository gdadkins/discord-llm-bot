...
import { batchValidate, validate } from '../../utils/validation';
import { RATE_LIMITER_CONSTANTS } from '../../utils/constants';
import { withTimeout } from '../../utils/timeoutUtils';
import { handleDataStoreOperation } from '../../utils/ErrorHandlingUtils';
import {
  IRateLimiter,
  RateLimitStatus
} from '../interfaces/RateLimitingInterfaces';
...
export class RateLimiter extends BaseService implements IRateLimiter {
  ...
  constructor(
    ...
    this.stateDataStore = dataStoreFactory.createStateStore<RateLimitState>(
      this.stateFile,
      stateValidator,
      { maxRetries: 0, retryDelayMs: 50 } // Fail fast, don't block
    );
  }
  ...
  // Circuit breaker for disk persistence
  private persistenceFailures = 0;
  private persistenceCircuitOpen = false;
  private lastPersistenceRejection = 0;
  private readonly PERSISTENCE_COOLDOWN = 300000; // 5 minutes

  private async atomicWriteState(state: RateLimitState): Promise<void> {
    // Check circuit breaker
    if (this.persistenceCircuitOpen) {
      if (Date.now() - this.lastPersistenceRejection > this.PERSISTENCE_COOLDOWN) {
        this.persistenceCircuitOpen = false;
        this.persistenceFailures = 0;
        logger.info('RateLimiter persistence circuit breaker reset');
      } else {
        return; // Skip write
      }
    }
    ...
    try {
      // Use a shorter timeout (5s) for rate limiter state - it's okay to lose some state updates
      // rather than blocking the bot logic
      await handleDataStoreOperation(
        () => this.stateDataStore.save(state),
        undefined,
        { context: 'rateLimiter-saveState' },
        { timeout: 5000 } // Hard timeout for persistence
      );
      ...
      if (this.persistenceFailures >= 5) {
        this.persistenceCircuitOpen = true;
        this.lastPersistenceRejection = Date.now();
        logger.warn('RateLimiter persistence circuit breaker opened - disabling disk writes for 5m');
      }
    }
  }
