...
exports.runEnhancedContextBenchmarks = runEnhancedContextBenchmarks;
const base_1 = require("./base");
const contextManager_1 = require("../../src/services/contextManager");
async function runEnhancedContextBenchmarks() {
    const suite = new base_1.BenchmarkSuite();
    // Test data generation
    const generateLargeContext = (items) => {
        const contexts = [];
    ...
    suite.add('ContextManager - Relevance-based context selection', () => {
        ...
        const highRelevancePreserved = highRelevanceItems.every(item => context.includes(item.substring(0, 20)));
    ...
    suite.add('ContextManager - Long-term memory optimization', () => {
        ...
        for (let minute = 0; minute < 60; minute++) {
            ...
            if (minute % 10 === 0) {
                const stats = manager.getMemoryStats();
                if (minute === 0)
                    totalMemoryBefore = stats.totalMemoryUsage;
...
//# sourceMappingURL=contextEnhanced.bench.js.map