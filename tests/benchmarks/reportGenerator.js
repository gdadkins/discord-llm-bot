...
exports.BenchmarkReportGenerator = void 0;
const fs_1 = require("fs");
const path_1 = require("path");
class BenchmarkReportGenerator {
    outputDir;
    constructor(outputDir = './benchmark-reports') {
        this.outputDir = outputDir;
        if (!(0, fs_1.existsSync)(this.outputDir)) {
            (0, fs_1.mkdirSync)(this.outputDir, { recursive: true });
        }
    }
    generateReport(benchmarks, memoryProfiles, previousReport) {
        ...
        if (previousReport && (0, fs_1.existsSync)(previousReport)) {
        ...
        const paths = {
            json: (0, path_1.join)(this.outputDir, `benchmark-report-${timestamp_clean}.json`),
            html: (0, path_1.join)(this.outputDir, `benchmark-report-${timestamp_clean}.html`),
            markdown: (0, path_1.join)(this.outputDir, `benchmark-report-${timestamp_clean}.md`),
            csv: (0, path_1.join)(this.outputDir, `benchmark-data-${timestamp_clean}.csv`)
        };
        Object.entries(reports).forEach(([format, content]) => {
            (0, fs_1.writeFileSync)(paths[format], content);
        });
    ...
    calculatePerformanceGrade(avgOpsPerSec, avgP95) {
    ...
    performAnalysis(benchmarks, memoryProfiles) {
    ...
    assessRegressionRisk(benchmarks) {
        const criticalIssues = benchmarks.filter(b => b.opsPerSecond < 100 || b.p95 > 100 || b.heapUsed > 200 * 1024 * 1024);
        if (criticalIssues.length > benchmarks.length * 0.1)
            return 'high';
        if (criticalIssues.length > 0)
            return 'medium';
        return 'low';
    }
    generateRecommendations(benchmarks, memoryProfiles) {
...
|-----------|---------|----------|----------|----------|-------------|
${data.benchmarks.map(b => `| ${b.name} | ${b.opsPerSecond.toFixed(2)} | ${b.p50.toFixed(3)} | ${b.p95.toFixed(3)} | ${b.p99.toFixed(3)} | ${(b.heapUsed / 1024 / 1024).toFixed(2)} |`).join('\n')}
...
`;
    ...
    generateCSVReport(data) {
        ...
        const rows = data.benchmarks.map(b => `"${b.name}",${b.operations},${b.duration},${b.opsPerSecond},${b.avgTimePerOp},${b.minTime},${b.maxTime},${b.p50},${b.p95},${b.p99},${b.memoryUsed},${b.heapUsed},${b.external},${b.gcRuns}`);
        return [headers, ...rows].join('\n');
    }
    generateComparison(current, previousFile) {
        try {
            const previous = JSON.parse((0, fs_1.readFileSync)(previousFile, 'utf8'));
...
exports.BenchmarkReportGenerator = BenchmarkReportGenerator;
