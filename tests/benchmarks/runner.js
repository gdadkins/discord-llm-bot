...
const contextManager_bench_1 = require("./contextManager.bench");
const messageSplitter_bench_1 = require("./messageSplitter.bench");
const rateLimiter_bench_1 = require("./rateLimiter.bench");
const roastProbability_bench_1 = require("./roastProbability.bench");
class BenchmarkRunner {
    modules = [
        {
            name: 'context',
            runner: contextManager_bench_1.runContextManagerBenchmarks,
            description: 'Context Manager performance benchmarks'
        },
        {
            name: 'splitter',
            runner: messageSplitter_bench_1.runMessageSplitterBenchmarks,
            description: 'Message Splitter performance benchmarks'
        },
        {
            name: 'ratelimiter',
            runner: rateLimiter_bench_1.runRateLimiterBenchmarks,
            description: 'Rate Limiter I/O performance benchmarks'
        },
        {
            name: 'roast',
            runner: roastProbability_bench_1.runRoastProbabilityBenchmarks,
    ...
    async run(options = {}) {
        ...
        for (const suite of suitesToRun) {
            ...
            try {
                ...
                const suiteReportPath = (0, path_1.join)(outputDir, `${suite.name}-${timestamp}.json`);
        ...
        const combinedReportPath = (0, path_1.join)(outputDir, `combined-${timestamp}.json`);
        ...
        if (options.compare && (0, fs_1.existsSync)(options.compare)) {
    ...
    compareResults(currentResults, previousFile) {
        try {
            const previousData = JSON.parse((0, fs_1.readFileSync)(previousFile, 'utf8'));
            const previousResults = previousData.results || previousData;
            console.log('\n' + '='.repeat(80));
