...
export async function runMessageSplitterBenchmarks() {
  const suite = new BenchmarkSuite();

  // Test data generation
  const generateMessage = (paragraphs: number, wordsPerParagraph: number): string => {
  ...
  const generateCodeBlock = (lines: number): string => {
    let code = '```typescript\n';
