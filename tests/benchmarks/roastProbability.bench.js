...
exports.runRoastProbabilityBenchmarks = runRoastProbabilityBenchmarks;
const base_1 = require("./base");
const gemini_1 = require("../../src/services/gemini");
async function runRoastProbabilityBenchmarks() {
    ...
    class BenchmarkGeminiService extends gemini_1.GeminiService {
        constructor() {
            super(testApiKey);
        }
        // Override shouldRoast to make it public for benchmarking
        testShouldRoast(userId, message = '', serverId) {
            return this.shouldRoast(userId, message, serverId);
        }
        // Override helper methods for testing
        testGetConsecutiveBonus(questionCount) {
            return this.getConsecutiveBonus(questionCount);
        }
        testCalculateComplexityModifier(message) {
            return this.calculateComplexityModifier(message);
        }
        testGetTimeBasedModifier() {
            return this.getTimeBasedModifier();
        }
        testGetMoodModifier(questionCount) {
            return this.getMoodModifier(questionCount);
        }
        testUpdateRoastDebt(userId, serverId) {
            return this.updateRoastDebt(userId, serverId);
        }
        testGetServerInfluenceModifier(serverId) {
            return this.getServerInfluenceModifier(serverId);
        }
        ...
        resetRoastingState() {
            ...
            this.userQuestionCounts = new Map();
        }
    }
    const geminiService = new BenchmarkGeminiService();
    // Test message generation
    const generateMessage = (complexity) => {
        switch (complexity) {
            case 'simple':
    ...
    moods.forEach(mood => {
        suite.add(`RoastProbability - mood modifier (${mood})`, () => {
            ...
            geminiService.roastingState.botMood = mood;
    ...
    suite.add('RoastProbability - chaos mode active', () => {
        geminiService.resetRoastingState();
        // Enable chaos mode
        geminiService.roastingState.chaosMode = {
            active: true,
            multiplier: 1.5,
    ...
    suite.add('RoastProbability - sustained decision making', () => {
        ...
        for (let i = 0; i < 50; i++) {
            ...
            const message = generateMessage(['simple', 'medium', 'complex'][i % 3]);
            geminiService.testShouldRoast(userId, message, serverId);
        }
        memoryProfiler.stop();
...
//# sourceMappingURL=roastProbability.bench.js.map